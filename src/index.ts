...
let socket,currentId='',revision=0,zoom=1,drawing=false,lastPoint=null,sentPoint=null,lastSentAt=0,panning=false,panStart=null;
...
function sendStroke(from,to){if(!socket||socket.readyState!==1)return;socket.send(JSON.stringify({type:'stroke',strokeId:crypto.randomUUID(),from,to,size:Number($('size').value),color:$('color').value}))}
...
async function openCanvas(){currentId=$('canvasId').value.trim();if(!/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/.test(currentId)){status('Invalid canvas ID',true);return}history.replaceState({},'',location.pathname+'?canvas='+encodeURIComponent(currentId));status('Opening…');try{const response=await api('',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:currentId,width:Number($('width').value),height:Number($('height').value),background:$('background').value})});const meta=await response.json();canvas.width=meta.width;canvas.height=meta.height;$('width').value=meta.width;$('height').value=meta.height;revision=meta.revision;$('title').textContent='— '+meta.name;setZoom(zoom);await loadImage('/api/canvases/'+encodeURIComponent(currentId)+'/image?v='+revision);connect();await listSnapshots();status('Connected')}catch(error){status(error.message,true)}}
function connect(){if(socket)socket.close();const protocol=location.protocol==='https:'?'wss:':'ws:';socket=new WebSocket(protocol+'//'+location.host+'/api/canvases/'+encodeURIComponent(currentId)+'/connect');socket.onopen=()=>status('Connected');socket.onclose=()=>{status('Disconnected',true);setTimeout(()=>{if(currentId)connect()},1500)};socket.onmessage=async event=>{const message=JSON.parse(event.data);if(message.type==='welcome'){revision=message.revision;canvas.width=message.width;canvas.height=message.height;setZoom(zoom);await loadImage(message.imageUrl)}else if(message.type==='stroke'){revision=message.revision;draw(message.from,message.to,message.size,message.color)}else if(message.type==='reset'){revision=message.revision;await loadImage(message.imageUrl);status('Snapshot restored')}else if(message.type==='restore-started')status('Restoring…');else if(message.type==='snapshot-created')listSnapshots();else if(message.type==='stroke-rejected'){status('Stroke rejected; reloading',true);await loadImage('/api/canvases/'+encodeURIComponent(currentId)+'/image?v='+revision)}else if(message.type==='server-error')status('Canvas persistence is retrying…',true)}}
canvas.addEventListener('pointerdown',event=>{if(event.button!==0||panning)return;drawing=true;lastPoint=point(event);sentPoint=lastPoint;lastSentAt=performance.now();canvas.setPointerCapture(event.pointerId);draw(lastPoint,lastPoint,Number($('size').value),$('color').value);sendStroke(lastPoint,lastPoint)});
canvas.addEventListener('pointermove',event=>{if(!drawing||!lastPoint||!sentPoint)return;const next=point(event);draw(lastPoint,next,Number($('size').value),$('color').value);lastPoint=next;const now=performance.now();if(now-lastSentAt>=32){sendStroke(sentPoint,next);sentPoint=next;lastSentAt=now}});
...
const initial=new URLSearchParams(location.search).get('canvas');if(initial)$('canvasId').value=initial;openCanvas();
</script>
</body></html>`;
