...
async function openCanvas(){currentId=$('canvasId').value.trim();if(!/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/.test(currentId)){status('Invalid canvas ID',true);return}history.replaceState({},'','/canvas/'+encodeURIComponent(currentId));status('Opening…');try{const response=await api('',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:currentId,width:Number($('width').value),height:Number($('height').value),background:$('background').value})});const meta=await response.json();canvas.width=meta.width;canvas.height=meta.height;$('width').value=meta.width;$('height').value=meta.height;canvasBackground=meta.background;revision=meta.revision;$('title').textContent='— '+meta.name;setZoom(zoom);await loadImage('/api/canvases/'+encodeURIComponent(currentId)+'/image?v='+revision);connect();await listSnapshots();status('Connected')}catch(error){status(error.message,true)}}
...
$('openBtn').onclick=openCanvas;$('snapshotBtn').onclick=saveSnapshot;$('forkBtn').onclick=forkCanvas;$('brushBtn').onclick=()=>selectTool('brush');$('eraserBtn').onclick=()=>selectTool('eraser');$('size').oninput=()=>$('sizeValue').textContent=$('size').value;$('zoomIn').onclick=()=>setZoom(zoom*1.25);$('zoomOut').onclick=()=>setZoom(zoom/1.25);$('zoomReset').onclick=()=>setZoom(1);
